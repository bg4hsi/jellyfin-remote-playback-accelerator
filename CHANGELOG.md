...
- 连续 3 个 30 秒异常窗口后，先确认 NAS 局域网辅助服务正常，再仅重连 `jellyfin-prefetch-tunnel`；
- 空闲、暂停、遥测缺失或连接身份不明确时不重连；
- 10 分钟冷却、每小时最多 2 次重连，重连预算在动作前持久化；
- 固定服务白名单和单一 `18097` 转发校验，不修改 DSM、Drive、播放隧道、Xray、nginx 或缓存；
- 不扩大隧道密钥权限，不新增公网健康接口。
