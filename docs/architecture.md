...
worker 同时读取玩家状态和家庭辅助服务状态。家庭端仍在转码时，只预取到 `safe_prefetch_max`；转码结束后，目标上限切换为 `last_generated`，把剩余已生成分片逐步搬入 VPS。
...
```text
最近有分片请求 -> PLAYING -> live prefetch
NAS 转码结束   -> PLAYING -> drain 到 last_generated
长时间无请求   -> STALE   -> 停止新增预取
新分片请求     -> PLAYING -> 恢复
```
...
3. 开源版因此不执行“猜测关闭后精确删除”。Nginx 开源版的文件缓存也不提供按业务会话可靠清理的内置接口。
